...
module.exports = (homebridge) => {
  ...
  // Path to settings.
  storagePath = path.join(homebridge.user.storagePath(), 'persist', 'MitsubishiAirconAUNZ.json')

  // Register the platform.
  homebridge.registerPlatform(packageJson.name, "MitsubishiAirconAUNZ", MitsubishiAirconAUNZ, true)
}

class MitsubishiAirconAUNZ {
  ...
  async _init() {
    ...
    for (const [uuid, accessory] of this.accessories) {
      if (!accessory.reachable) {
        this.log(`Deleting non-available accessory ${uuid}`)
        this.accessories.delete(uuid)
        this.api.unregisterPlatformAccessories(packageJson.name, "MitsubishiAirconAUNZ", [accessory])
  ...
  async _addAccesory(aircon, uuid) {
    ...
    if (!registered) {
      ...
      this.api.registerPlatformAccessories(packageJson.name, "MitsubishiAirconAUNZ", [accessory])
  ...
  async _buildAccessory(aircon, accessory) {
    ...
    const service = accessory.getService(hap.Service.HeaterCooler) ||
      	...
      	if (aircon.outsideTemp !== null) {
			//has a temperature sensor outside. Let's show that info
			const tempSensorService = accessory.getService(hap.Service.TemperatureSensor)  || accessory.addService(hap.Service.TemperatureSensor)
		
			tempSensorService.getCharacteristic(hap.Characteristic.CurrentTemperature).updateValue(aircon.outsideTemp)
			tempSensorService.getCharacteristic(hap.Characteristic.Name).updateValue("Outside Temperature")
		}
	...
	//TODO
    //service.getCharacteristic(hap.Characteristic.SwingMode);
    ...
    .on('get', async (callback) => {
      try {
        await this._updateIfNeeded(aircon)
        callback(null, aircon.setfan)
      } catch (e) {
        this.log(`Failed to get target temperature: ${e}`)
        callback(e)
      }
    })
    .on('set', async (value, callback) => {
      try {
        ...
        this.log(`Unable to set fan speed: ${e}`)
        callback(e)
      }
    })
